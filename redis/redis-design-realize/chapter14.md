...
##### 实现
  ...
  - 1、从服务器接受到客户端发送的slaveof命令。
  - 2、是否为第一次复制？如果是执行3，否则执行4
  - 3、向服务器发送PSYNC ? -1, 主服务器返回+FULLERSYNC runid offset 执行完整同步操作。
  - 4、向主服务器发送 PSYNC runid offset
  - 5、主服务器是否返回+CONTINUE，如果是就执行步骤6，否则执行步骤3.
  - 6、执行部分同步操作

### 其他

  在复制操作刚开始时，从服务器会成为主服务器的客户端，并通过向主服务器发送命令请求执行同步操作。在复制后期，则它们成为相互的客户端。

  从服务器会通过心跳检测以及命令丢失检测确保主从的一致性。